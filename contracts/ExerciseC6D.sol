...
contract ExerciseC6D {
    ...
    // Number of oracles that must respond for valid status (Threshold)
    ...
    event FlightStatusInfo(uint index, string flight, uint256 timestamp, uint8 status, bool verified);
                        ...
                        external
    {
        ...
        if (oracleResponses[key].responses[statusId].length >= MIN_RESPONSES) {
            ...
            emit FlightStatusInfo(index, flight, timestamp, statusId, true);
            ...
            // CODE EXERCISE 3: Announce to the world that verified flight status information is available
            emit FlightStatusInfo(index, flight, timestamp, statusId, false);
        }
    }
